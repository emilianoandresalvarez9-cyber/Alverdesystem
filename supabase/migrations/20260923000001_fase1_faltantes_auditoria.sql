...
-- Columnas de audit_history según schema de Fase 0:
--   entity, entity_id, field, old_value, new_value, user_id, occurred_at
-- ==============================================================================

-- 1. TABLA missing_items (RF-49/50)
...
CREATE POLICY "Autenticados pueden reportar faltantes"
    ON public.missing_items FOR INSERT TO authenticated WITH CHECK (true);
...
-- Trigger updated_at automatico para missing_items
...
-- 2. FUNCION TRIGGER DE AUDITORIA (RF-58)
-- Itera columnas con jsonb_each_text, solo registra las que cambiaron.
-- SECURITY DEFINER: persiste el log aunque el usuario no tenga permisos directos.
    ...
    LOOP
        ...
        IF v_old_val IS DISTINCT FROM v_new_val THEN
            INSERT INTO public.audit_history (
                ...
                TG_TABLE_NAME, v_entity_id, v_key,
                to_jsonb(v_old_val), to_jsonb(v_new_val),
                v_user_id, now()
            );
        END IF;
    END LOOP;
...
CREATE TRIGGER trg_audit_products
    AFTER UPDATE ON public.products FOR EACH ROW EXECUTE FUNCTION public.log_audit_change();

DROP TRIGGER IF EXISTS trg_audit_brands ON public.brands;
CREATE TRIGGER trg_audit_brands
    AFTER UPDATE ON public.brands FOR EACH ROW EXECUTE FUNCTION public.log_audit_change();

DROP TRIGGER IF EXISTS trg_audit_categories ON public.categories;
CREATE TRIGGER trg_audit_categories
    AFTER UPDATE ON public.categories FOR EACH ROW EXECUTE FUNCTION public.log_audit_change();

DROP TRIGGER IF EXISTS trg_audit_labels ON public.labels;
CREATE TRIGGER trg_audit_labels
    AFTER UPDATE ON public.labels FOR EACH ROW EXECUTE FUNCTION public.log_audit_change();
...
CREATE TRIGGER trg_audit_product_presentations
    AFTER UPDATE ON public.product_presentations FOR EACH ROW EXECUTE FUNCTION public.log_audit_change();
